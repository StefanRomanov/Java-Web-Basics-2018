...
public class DispatcherSolet  extends BaseHttpSolet {
    ...
    private ControllerLoadingService cls;
...
//
            //content.append("<h1>" + e.getMessage() + "</h1>");
//
            //content.append("<p>");
...
//
    ...
    private ControllerActionPair getControllerActionPairCandidate(HttpSoletRequest request) {
        ...
        ControllerActionPair candidateControllerActionPair = this.cls.getLoadedControllersAndActions()
    ...
    public void init(SoletConfig soletConfig) {
        ...
        this.cls = new ControllerLoadingService();

        this.templateEngine = new TemplateEngine(
                soletConfig.getAttribute("application-folder") +
                        "resources" +
                        File.separator +
                        "templates" +
                        File.separator);
        ...
        try {
            this.cls.loadControllerActionHandlers(this.applicationClassesFolderPath);
        } catch (NoSuchMethodException | InstantiationException | InvocationTargetException | IllegalAccessException | ClassNotFoundException | IOException e) {
            e.printStackTrace();
        }
    }
