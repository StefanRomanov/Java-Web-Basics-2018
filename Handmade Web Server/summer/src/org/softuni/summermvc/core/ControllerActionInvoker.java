...
public class ControllerActionInvoker {
    ...
    private boolean isString(Parameter parameter){
        ...
        return parameter.getType().getSimpleName().equals(String.class.getSimpleName());
    }
    ...
    private Object[] getActionArguments(Method action, Set<Object> parameters) throws InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        ...
        Iterator<Object> parameterIterator = parameters.iterator();
        ...
        for (int i = 0; i < actionArguments.length; i++) {
            ...
            if(this.isPathVariable(currentParameter) && parameterIterator.hasNext()){
                Object pathVariable = parameterIterator.next();

                actionArguments[i] = this.parseValue(currentParameter.getType(), pathVariable.toString());

            }else if(this.isModel(currentParameter)){
                actionArguments[i] = this.dependencyContainer
                ...
                this.populateBindingModel(bindingModel,
                        (HttpSoletRequest) this.dependencyContainer.getObject(HttpSoletRequest.class.getSimpleName()));
